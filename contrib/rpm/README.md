...
The RPM spec file provided here is for Neoxa-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://neoxa.net/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Neoxa git repository](https://github.com/NeoxaChain/Neoxa) but are not part of
...
Currently only `Source10` is used, for the example `neoxa.conf` file.
...
in the neoxa git repository. At present only an SVG version of the Neoxa
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the neoxa
...
needed when 0.12.1 is released, a proper fix is already in the Neoxa git
master branch.
...
idea to build Neoxa Core against a static version of that library compiled
...
The `neoxa.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `neoxa-server` package. At this point, most RPM based
...
The `neoxa-server` package should have SELinux support. How to properly do
...
Neoxa-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/neoxa/). He has been
packaging Neoxa for Fedora at least since 2012.
...
4. I renamed the `neoxa` package that contains the Qt GUI to `neoxa-core` as
that appears to be how the general population refers to it, in contrast to
`neoxa-xt` or `neoxa-classic`. I wanted to make sure the general population
