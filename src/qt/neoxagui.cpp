...
const std::string NeoxaGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString NeoxaGUI::DEFAULT_WALLET = "~Default";

NeoxaGUI::NeoxaGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void NeoxaGUI::createActions()
{
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to NEOX addresses"));
...
void NeoxaGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/neoxacointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* neoxaLabelLayout = new QVBoxLayout(toolbarWidget);
        neoxaLabelLayout->addWidget(label);
        neoxaLabelLayout->addWidget(toolbar);
        neoxaLabelLayout->setDirection(QBoxLayout::TopToBottom);
        neoxaLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Neoxa Market Price"));
        ...
        // Get the latest Neoxa release and let the user know if they are using the latest version
        // Network request code for the header widget
...
void NeoxaGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool NeoxaGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool NeoxaGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void NeoxaGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void NeoxaGUI::setWalletActionsEnabled(bool enabled)
{
...
void NeoxaGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void NeoxaGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void NeoxaGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void NeoxaGUI::showDebugWindowActivateInformation()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_INFO);
    showDebugWindow();
}

void NeoxaGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void NeoxaGUI::showDebugWindowPeerList()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_PEERS);
    showDebugWindow();
}

void NeoxaGUI::showDebugWindowNetworkTraffic()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_GRAPH);
    showDebugWindow();
}

void NeoxaGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void NeoxaGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void NeoxaGUI::openClicked()
{
...
void NeoxaGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void NeoxaGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void NeoxaGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void NeoxaGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void NeoxaGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void NeoxaGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** NEOXA START */
void NeoxaGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void NeoxaGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void NeoxaGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void NeoxaGUI::gotoRestrictedAssetsPage()
{
...
void NeoxaGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to neoxa network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void NeoxaGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void NeoxaGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void NeoxaGUI::updateHeadersSyncProgressLabel()
{
...
void NeoxaGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void NeoxaGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Neoxa"); // default title
...
void NeoxaGUI::changeEvent(QEvent *e)
{
...
void NeoxaGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void NeoxaGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void NeoxaGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "NEOX")
        msg += tr("Amount: %1\n").arg(NeoxaUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(NeoxaUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void NeoxaGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void NeoxaGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool NeoxaGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool NeoxaGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void NeoxaGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void NeoxaGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void NeoxaGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void NeoxaGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void NeoxaGUI::detectShutdown()
{
...
void NeoxaGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void NeoxaGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void NeoxaGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(NeoxaGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
static bool ThreadSafeMnemonic(NeoxaGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void NeoxaGUI::subscribeToCoreSignals()
{
...
void NeoxaGUI::unsubscribeFromCoreSignals()
{
...
void NeoxaGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void NeoxaGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<NeoxaUnits::Unit> units = NeoxaUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const NeoxaUnits::Unit unit : units)
    {
    	#ifndef QTversionPreFiveEleven
        	max_width = qMax(max_width, fm.horizontalAdvance(NeoxaUnits::name(unit)));
        #else
        	max_width = qMax(max_width, fm.width(NeoxaUnits::name(unit)));
        #endif
    }
...
void UnitDisplayStatusBarControl::createContextMenu(const PlatformStyle *platformStyle)
{
    ...
    {
        QAction *menuAction = new QAction(QString(NeoxaUnits::name(u)), this);
...
{
    setText(NeoxaUnits::name(newUnits));
}
...
void NeoxaGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=NEOXBTC"));
    networkManager->get(*request);
}

#ifdef ENABLE_WALLET
void NeoxaGUI::mnemonic()
{
...
void NeoxaGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/NeoxaChain/Neoxa/releases"));
